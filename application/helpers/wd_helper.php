...
if ( ! function_exists('element')) {
	...
	function img_src($src,$path,$replace="General/not-found.png"){
		$ci = &get_instance();

		if(substr($path, -1)!='/') $path.='/';

		if ($src=='') {
			$img=$ci->config->item('assets').$replace;	
		}else{
	...
	function get_wd_options($name){
		$ci = &get_instance();
		$data = $ci->wd_db->get_row('wd_options',array('name'=> $name));
