...
class Dashboard_m extends CI_Model
{
	...
	public function where_superadmin(){
		...
		$this->db->distinct();
		$this->db->select('user_id , group_id , wd_users.username , wd_users.last_login , wd_groups.name');
		...
		$this->db->or_where('wd_users_groups.group_id =','2');
	...
	public function where_not_superadmin(){
		...
		$this->db->join('wd_groups','wd_users_groups.group_id=wd_groups.id');
		$this->db->where('wd_users_groups.group_id !=','1');
		$this->db->where('wd_users_groups.group_id =','2');
