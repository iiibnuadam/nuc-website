...
if($privilege['U']=="1" || $privilege['D']=="1"){
	...
	if($privilege['U']=="1"){
		$action .= '<li><a class="btn-deactive" href="#">Deactive</a></li>';
	}
	if($privilege['U']=="1"){
		$action .= '<li><a class="btn-active" href="#">Active</a></li>';
	}
...
$(function() {
	...
	var dt = $("#dt_basic").DataTable({
        ...
        columns: [
			...
			{ data: "u.active", render: function ( data, type, row ) {
                  if (data == '1') {
                    return '<span class="badge bg-green">Active</span>';
                    }
                  else {
                    return '<span class="badge bg-red"> Deactive</span>';
                    }
                }
            },
			{
	...
	$("#dt_basic tbody").on("click", ".btn-deactive", function() {
		alert('Akun akan di non-aktiv');
		var tr = $(this).closest("tr");
		var recordId = tr.attr("id");
		window.location =  JS_BASE_URL + "<?php echo this_module();?>/deactive?id=" + recordId;
		console.log(recordId);
	});


	$("#dt_basic tbody").on("click", ".btn-active", function() {
		alert('Akun akan di aktiv kan kembali');
		var tr = $(this).closest("tr");
		var recordId = tr.attr("id");
		window.location =  JS_BASE_URL + "<?php echo this_module();?>/active?id=" + recordId;
		console.log(recordId);
	});
